#!/usr/bin/env python
"""TPOT + AutoGluon Runner (Python 3.11 – env-tpa)

This script is meant to be executed from the *env-tpa* pyenv virtual
environment. It expects that `tpot==1.0.0` and `autogluon.tabular`
along with their supported dependency stacks is available.

Usage
-----
python run_tpot_ag.py \
    --predictors DataSets/3/predictors_Hold\ 1\ Full_20250527_151252.csv \
    --target-column y \
    --artifacts-dir 05_outputs/dataset_name \
    [--timeout 3600]

Parameters
~~~~~~~~~~
--predictors       Path to the predictors CSV file.
--target-column    Name of the target column *within* the predictors CSV.
--target-path      Alternative to --target-column: a CSV containing the
target values. If provided, the target column will be taken as the *only*
column from that file.
--artifacts-dir    Directory where model.pkl, metrics.json, and run.log will
be saved. It will be created if missing.
--timeout          Optional wall-clock limit per Auto-Sklearn run.
"""
from __future__ import annotations

import argparse
import json
import random
import sys
import time
from datetime import datetime
from pathlib import Path
from typing import Any, Dict

import joblib
import numpy as np
import pandas as pd
from tpot import TPOTRegressor # Changed import
from rich.console import Console
from rich.tree import Tree
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from sklearn.model_selection import RepeatedKFold

RANDOM_STATE = 42
CV_REPEATS = 3
CV_SPLITS = 5
N_JOBS_CV = 1 # Changed to 1, TPOT handles its own parallelism internally

console = Console(highlight=False, record=True)

def _rmse(y_true, y_pred):
    return np.sqrt(mean_squared_error(y_true, y_pred))

def _parse_args() -> argparse.Namespace:  # noqa: D401
    parser = argparse.ArgumentParser(description="Run TPOT/AutoGluon (regression)") # Changed description
    parser.add_argument("--predictors", required=True, help="Path to predictors CSV")
    parser.add_argument("--target-column", help="Name of target column in predictors CSV")
    parser.add_argument("--target-path", help="Separate CSV containing target.")
    parser.add_argument("--artifacts-dir", required=True, help="Output directory for artifacts")
    parser.add_argument("--timeout", type=int, default=3600, help="Wall-clock limit (s)")
    args = parser.parse_args()

    # Convert relative paths to absolute paths
    args.predictors = str(Path(args.predictors).resolve())
    if args.target_path:
        args.target_path = str(Path(args.target_path).resolve())

    return args

def _load_data(args: argparse.Namespace):  # noqa: D401
    X: pd.DataFrame
    y: pd.Series

    if args.target_path:
        X = pd.read_csv(args.predictors, index_col=None)
        y = pd.read_csv(args.target_path, index_col=None).iloc[:, 0]
    else:
        if not args.target_column:
            console.print("[red]Either --target-column or --target-path must be provided")
            sys.exit(1)
        df = pd.read_csv(args.predictors, index_col=None)
        y = df.pop(args.target_column)
        X = df
    return X, y

def main() -> None:  # noqa: D401
    args = _parse_args()
    artifacts_dir = Path(args.artifacts_dir)
    artifacts_dir.mkdir(parents=True, exist_ok=True)

    random.seed(RANDOM_STATE)
    np.random.seed(RANDOM_STATE)

    root = Tree("[bold cyan]TPOT + AutoGluon Runner[/bold cyan]") # Changed title

    # ------------------------------------------------------------------
    # Data loading
    # ------------------------------------------------------------------
    root.add("Loading data…")
    print(f"Current working directory: {Path.cwd()}") # Added for debugging
    X, y = _load_data(args)
    root.add(f"X shape = {X.shape}, y shape = {y.shape}")

    # ------------------------------------------------------------------
    # Model fitting and Cross-validation evaluation
    # ------------------------------------------------------------------
    rkf = RepeatedKFold(n_splits=CV_SPLITS, n_repeats=CV_REPEATS, random_state=RANDOM_STATE)

    cv_node = root.add("[bold]5×3 Repeated K-Fold evaluation…[/bold]")
    r2_scores, rmse_scores, mae_scores = [], [], []
    
    # Store the best performing pipeline from all folds
    best_overall_model = None
    best_overall_score = -np.inf # For R2, higher is better

    for fold_idx, (train_idx, test_idx) in enumerate(rkf.split(X), 1):
        X_tr, X_te = X.iloc[train_idx], X.iloc[test_idx]
        y_tr, y_te = y.iloc[train_idx], y.iloc[test_idx]
        
        # Instantiate TPOTRegressor for each fold
        tpot = TPOTRegressor(
            generations=5, # Example: Number of iterations to run the pipeline optimization process
            population_size=20, # Example: Number of individuals to retain in the population
            cv=CV_SPLITS, # Internal CV for TPOT
            random_state=RANDOM_STATE,
            n_jobs=N_JOBS_CV, # Use N_JOBS_CV for TPOT internal parallelism
            verbose=5, # Set verbosity to 5 for full error messages
            max_time_mins=args.timeout / 60, # Pass timeout to TPOT in minutes
            config_dict='TPOT light' # Use a light configuration for regression
        )

        start_fit = time.perf_counter()
        tpot.fit(X_tr, y_tr)
        fit_seconds = time.perf_counter() - start_fit
        
        preds = tpot.predict(X_te)

        r2 = r2_score(y_te, preds)
        rmse = _rmse(y_te, preds)
        mae = mean_absolute_error(y_te, preds)

        r2_scores.append(r2)
        rmse_scores.append(rmse)
        mae_scores.append(mae)

        cv_node.add(f"Fold {fold_idx:02d}: R²={r2:.4f}  RMSE={rmse:.4f}  MAE={mae:.4f} (Fit time: {fit_seconds:.1f}s)")
        
        # Keep track of the best model across all folds
        if r2 > best_overall_score:
            best_overall_score = r2
            best_overall_model = tpot.fitted_pipeline_


    metrics: Dict[str, Any] = {
        "cv": {
            "r2_mean": float(np.mean(r2_scores)),
            "r2_std": float(np.std(r2_scores)),
            "rmse_mean": float(np.mean(rmse_scores)),
            "rmse_std": float(np.std(rmse_scores)),
            "mae_mean": float(np.mean(mae_scores)),
            "mae_std": float(np.std(mae_scores)),
        },
        "fit_seconds_per_fold_mean": float(np.mean([t['fit_seconds'] for t in cv_node.children if 'Fit time' in t.text])), # This needs to be captured per fold
        "timestamp": datetime.utcnow().isoformat(),
    }

    # ------------------------------------------------------------------
    # Persist artifacts
    # ------------------------------------------------------------------
    if best_overall_model:
        model_path = artifacts_dir / "tpot_wrapper_champion.pkl"
        joblib.dump(best_overall_model, model_path)
        root.add(f"[blue]Champion Model saved → {model_path}")

    metrics_path = artifacts_dir / "metrics_tpot.json" # Changed metrics filename
    metrics_path.write_text(json.dumps(metrics, indent=2))
    root.add(f"[blue]Metrics saved → {metrics_path}")

    log_path = artifacts_dir / "tpot_run.log" # Changed log filename
    log_path.write_text(console.export_text())
    root.add(f"[blue]Verbose log → {log_path}")

    console.print(root)

if __name__ == "__main__":
    main() 
